...
from Beta_VAE import Beta_VAE_Agent, QVAE_Factor_Agent

cuda_dic = {'zero':0, 'one':1, 'two':2, 'three':3, 'four':4, 'five':5, 'six':6, 'seven':7}


def train_once(repetition_index, bandwidth, N, T, P_f, P_x, P_c, W, linear_index, K, beta, lr, f_hidden_dim, lam, tau, AE_epoch_num, AE_factor_epoch_num, root_log_dir, seed):
    ...
    r_hat_total = AE_factor.network.forward(AE_factor.feature_test.to(AE_factor.network.device)).cpu().detach().numpy().mean(axis = 2)
    ...
    mean, log_std = AE_factor.network.Encoder(portfolio_test.to(AE_factor.network.device))
    latent_ = mean
...
def run(args):
    ...
    for repetition_index in range(repetition_num):
        R_total, R_pred = train_once(repetition_index, bandwidth, N, T, P_f, P_x, P_c, W, linear_index, K, beta, lr, f_hidden_dim, lam, tau, AE_epoch_num, AE_factor_epoch_num, root_log_dir, seed)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--log_name', type=str, default = 'QVAE_test')
