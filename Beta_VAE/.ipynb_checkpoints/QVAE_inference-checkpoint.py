...
class QVAE_Factor_Inference(object):
    def __init__(self, factor_matrix_path, model_path, K, f_hidden_dim, bandwidth, factor_list, tau = torch.Tensor([0.1, 0.3, 0.5, 0.7, 0.9]).unsqueeze(0).unsqueeze(1)):
    ...
    def load_data(self, log_period, permno_list, set_zero: Optional = None):
    ...
    def inference(self, log_period, permno_list, set_zero: Optional = None):
        ...
        with torch.no_grad():
            for j in networks.keys():
                ...
                AE_model_para = torch.load(f'{path}/Beta_VAE_best.pth', map_location = self.device)
                networks[j] = Quantile_VAE_Factor_Network(self.N, self.T, self.P, self.K, self.f_hidden_dim, self.tau.shape[-1], AE_model_para, )
                networks[j].load_state_dict(torch.load(f'{path}/QVAEF_best.pth', map_location = self.device))
                networks[j].eval()
                ...
                r_ = feature[:, :, -1]
                c_ = feature[:, :, :-1]
                latent_variable, _ = networks[j].Encoder(r_.to(self.device))
