...
from Network import AE_Factor_Network


class AE_Factor_Agent():
    def __init__(self, N, T, P, K, f_hidden_dim, model_para, learning_rate, seed, log_dir):
        ...
        np.random.seed(seed)
        '''
        network and optimatizer
        '''
        self.network = AE_Factor_Network(N, T, P, K, f_hidden_dim, model_para)
        self.network.train()
    ...
    def evaluate(self, i, epoch_num, lam):
        '''
        evaluate the performance of trained network periodically
        '''
        self.network.eval()
        with torch.no_grad():
            valid_losses = list()
        ...
        if valid_loss.item() < self.best_score:
            ...
            torch.save(self.network.state_dict(), os.path.join(self.log_dir, 'AEF_best.pth'))
            self.early_stopping_count = 0
        else:
