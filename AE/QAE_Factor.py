...
class QAE_Factor_Agent():
    def __init__(self, N, T, P, K, f_hidden_dim, tau, model_para, learning_rate, seed, log_dir):
        ...
        self.network = Quantile_AE_Factor_Network(N, T, P, K, f_hidden_dim, self.tau_num, model_para)
        self.network.train()
    ...
    def train(self, epoch_num, lam):
        print("Start training quantile network...")
        self.network.train()
        for i in range(epoch_num):
            ...
            for j, (x, y) in enumerate(self.train_loader):
                ...
                self.update_params(self.optimizer, loss, networks=[self.network.factor_loading_network])
            end_time = time.time()
            ...
            if i % 5 == 0:
                ...
                # torch.save(self.network.state_dict(), os.path.join(self.log_dir,'QAEF_final.pth'))
    ...
    def evaluate(self, i, epoch_num, lam):
        ...
        if valid_loss.item() < self.best_score:
            ...
            torch.save(self.network.state_dict(), os.path.join(self.log_dir, 'QAEF_best.pth'))
            self.early_stopping_count = 0
        else:
