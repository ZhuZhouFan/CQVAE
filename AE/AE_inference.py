...
class AE_Factor_Inference(object):
    ...
    def inference(self, log_period, permno_list, feature = None, set_zero = None):
        ...
        with torch.no_grad():
            for j in networks.keys():
                ...
                networks[j] = AE_Factor_Network(self.N, self.T, self.P, self.K, self.f_hidden_dim, AE_model_para)
                networks[j].load_state_dict(torch.load(f'{path}/AEF_best.pth', map_location = self.device))
                networks[j].eval()
