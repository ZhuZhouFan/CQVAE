...
class VAE_Factor_Inference(object):
    ...
    def inference(self, log_period, permno_list, set_zero: Optional = None):
        ...
        with torch.no_grad():
            for j in networks.keys():
                ...
                networks[j] = VAE_Factor_Network(self.N, self.T, self.P, self.K, self.f_hidden_dim, AE_model_para)
                networks[j].load_state_dict(torch.load(f'{path}/VAEF_best.pth', map_location = self.device))
                networks[j].eval()
                y_totals[j] = pd.DataFrame(data = networks[j].forward(feature.to(self.device)).cpu().numpy().squeeze(2),
