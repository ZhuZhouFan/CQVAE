...
import torch
from AE import AE_Agent, QAE_Factor_Agent
...
def run(args):
    cuda_dic = {'zero': 0, 'one': 1, 'two': 2, 'three': 3,
    ...
    for i in range(len(period_set)):
        ...
        data_list = Parallel(n_jobs=10)(delayed(load_mat)(factor_name, train_period, factor_matrix_path)
                                        for factor_name in tqdm(factor_list, desc='Loading factor matrixs'))
        permno_list = obtain_permno_list(
            'RET', train_period, '/data/QAE/xiu_factor_matrix_with_na')
        T = data_list[0].shape[0]
        N = len(permno_list)
        P = len(data_list)
        r = load_mat('RET', train_period, factor_matrix_path)[
            permno_list].values.transpose()
        C = np.zeros((N, T, P))
        for i in range(P):
        ...
        if not os.path.exists(os.path.join(log_name, '%s-%s' % (log_period[0], log_period[1]))):
            os.mkdir(os.path.join(log_name, '%s-%s' %
                     (log_period[0], log_period[1])))
        log_dir = os.path.join(
            log_name, '%s-%s' % (log_period[0], log_period[1]), str(args.seed)+'-'+str(time_))
        portfolio = np.zeros((P, T))
        for t in range(T):
            portfolio[:, t] = np.linalg.inv(C[:, t, :].transpose() @ C[:, t, :]) @ C[:, t, :].transpose() @ r[:, t]
        AE_agent = AE_Agent(input_dim=N,
                            latent_dim=K,
                            output_dim=N,
                            learning_rate=AE_lr,
                            seed=seed,
        ...
        AE_agent.load_data(feature=AE_feature, label=AE_label,
                           valid_size=1/3, test_size=0, num_cpu=0, batch_size=32)
        ...
        AE_factor = QAE_Factor_Agent(N=N,
                                     ...
                                     seed=seed,
                                     log_dir=log_dir)
        AE_factor.load_data(C=C, r=r, valid_size=1/3,
                            test_size=0, batch_size=32, num_cpu=0)
